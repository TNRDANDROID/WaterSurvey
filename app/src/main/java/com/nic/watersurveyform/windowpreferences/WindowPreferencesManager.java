package com.nic.watersurveyform.windowpreferences;/*
...
//  }
  ...
  public void applyEdgeToEdgePreference(Window window) {
    ...
    boolean edgeToEdgeEnabled = true;

    int statusBarColor = getStatusBarColor(true);
    int navbarColor = getNavBarColor(true);

    @SuppressLint("RestrictedApi") boolean lightBackground = isColorLight(
  ...
  private int getStatusBarColor(boolean isEdgeToEdgeEnabled) {
    if (isEdgeToEdgeEnabled && VERSION.SDK_INT < VERSION_CODES.M) {
      @SuppressLint("RestrictedApi") int opaqueStatusBarColor =
  ...
  private int getNavBarColor(boolean isEdgeToEdgeEnabled) {
    if (isEdgeToEdgeEnabled && VERSION.SDK_INT < VERSION_CODES.O_MR1) {
      @SuppressLint("RestrictedApi") int opaqueNavBarColor =
